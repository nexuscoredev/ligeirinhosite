(function () {
    ...
    const notifyPromoCount = (entries) => {
        const available = entries.filter((entry) => entry.item).length;
        deps?.onPromoCount?.(available, entries.length);
    };
